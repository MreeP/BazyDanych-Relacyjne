                        ...
                        <ul role="list" class="flex flex-1 flex-col gap-y-7">
                            <li>
                                <ul role="list" class="-mx-2 space-y-1">
                                    @foreach(menu(\App\Helpers\Menu\Contracts\MenuProvider::ADMIN) as $item)
                                        <x-admin::layout.app.menu-item :item="$item" />
                                    @endforeach
                                </ul>
                            </li>
                            <li class="-mx-6 mt-auto flex items-center justify-between px-6 py-3">
                                <div class="flex items-center justify-center space-x-3">
                                    ...
                                    <a href="{{ \Illuminate\Support\Facades\URL::route('admin.profile.edit') }}" class="flex flex-col justify-center text-sm font-semibold text-gray-900">
                                        <span aria-hidden="true">{{ \Illuminate\Support\Facades\Auth::user()->name }}</span>
                                        <span class="text-gray-600">{{ __('Your profile') }}</span>
                                    </a>
                                </div>

                                <form action="{{ route('admin.auth.logout') }}" method="POST" class="flex-shrink-0">
                                    @csrf
